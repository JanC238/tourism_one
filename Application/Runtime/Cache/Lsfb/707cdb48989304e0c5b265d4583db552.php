			...
			<div class="row">
				<div class="pageheader" style="padding:0px 10px 10px 10px; border-bottom:0px; background:#FFF">
					<div style="border-bottom:1px #D8DBDB solid; padding:5px;font: 12px/1.5 Microsoft YaHei,Helvitica,Verdana,Arial,san-serif;">
						<?php if(ACTION_NAME == 'edit'): ?><h3 style="margin:0;">出行保障修改</h3>
							<?php else: ?>
							<h3 style="margin:0;">出行保障添加</h3><?php endif; ?>
					</div>
				</div>
			</div>
					...
					<form class="Huiform" id="loginform" action="" method="post">
						<table width="100%" class="tabled m-top-md table-bordered" border='0'>
							...
							<tr>
								<td>名称</td>
								<td>
									<input type="text" class="form-control width200" value="<?php echo ($row["name"]); ?>" name="name">
								</td>
							</tr>
							<tr>
								<td>简介</td>
								<td>
									<input type="text" class="form-control width200" value="<?php echo ($row["intro"]); ?>" name="intro">
								</td>
							</tr>
							<tr>
								<td>选择分类</td>
								<td>
									<select name="pid" class="form-control width200 pid">
										...
										<?php if(is_array($clas)): $i = 0; $__LIST__ = $clas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cla): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cla["id"]); ?>"><?php echo ($cla["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</td>
							</tr>
							<!--<tr>-->
								...
								<!--<td>-->
									...
									<!--<?php if($row): ?>-->
										<!--<img src="/lsfb/tourism_one/Public/images/<?php echo ($row["image"]); ?>" id="img" width="200px" alt="">-->
										<!--<?php else: ?>-->
							...
							<tr>
								<td>时间</td>
								<td>
									<input type="datetime-local" class="form-control width200" value="<?php echo ($row["time"]); ?>" name="time">
								</td>
							</tr>
							<tr>
								<th></th>
...
<?php if($row['pid']): ?><script>
		$('.pid').val(["<?php echo ($row["pid"]); ?>"])
	</script><?php endif; ?>
<style src="/lsfb/tourism_one/Public/uploadify/uploadify.css"></style>
<script src="/lsfb/tourism_one/Public/uploadify/jquery.uploadify.min.js"></script>
<script>
    //>>使用uploadify初始化logo文件框
    $options = {
        swf: '/lsfb/tourism_one/Public/uploadify/uploadify.swf',
        uploader: '<?php echo U("Upload/imgUploadOne");?>',
        'buttonText': '  选择图片  ',
        ...
        onUploadSuccess: function (file_item, response) {
            ...
            if (response.status == 1) {
                ...
                $('#img').css('display', '');
                $('#img').attr('src', '/lsfb/tourism_one/Public/images/' + response.file_name);
            } else {
                alert('上传失败');
            }
        }
