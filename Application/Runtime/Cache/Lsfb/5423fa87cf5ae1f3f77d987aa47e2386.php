						...
						<div class="col-lg-12 col-sm-6">
							<table width="100%" class="tables m-top-md table-hover table-striped">
								...
								<?php if(is_array($rows)): $k = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($k % 2 );++$k;?><tr class="text-c">
										...
										<td><?php echo (date('Y-m-s H:i:s',$row["create_time"])); ?></td>
										<td>
											<?php if($row['status'] == 1): ?><a href="javascript:;" name="<?php echo ($row["id"]); ?>" class="btn btn-info status">已处理</a>
												<?php else: ?>
												<a href="javascript:;" name="<?php echo ($row["id"]); ?>" class="btn btn-infos status">未处理</a><?php endif; ?>
										</td>
...
<script>
	...
	$(".btndel").on("click",function(){
		if(confirm("确定要删除账号吗")){
			$.maskajax('post',"<?php echo U('/Order/del');?>",{"id":$(this).attr("name")},function(data){
				if(data == 0) {
					alert('删除出错了');
				}else {
                    location.reload();
		...
		<script>
            $('.status').click(function () {
                ...
                $.post('<?php echo U("/Order/changeStatus");?>', {id: id}, function (res) {
                    if(res == 1) {
                        alert('失败');
                    }else if(res == 2) {
                        alert('最多显示四个');
