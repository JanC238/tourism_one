					...
					<div class="row">
						<div class="pageheader" style="padding:0px 10px 10px 10px; border-bottom:0px; background:#FFF">
						    ...
						    <div style="width:100%; padding:10px 0 10px 10px; margin-top:10px;">
								<?php if(in_array('HomeContent/add',per()) OR user()['id'] == 1): ?><a href="<?php echo U('/HomeContent/add');?>" class="btn btn-add"><i class="fa fa-plus"></i> 添加</a><?php endif; ?>
    						</div>
						</div>
					</div>
						...
						<div class="col-lg-12 col-sm-6">
							<table width="100%" class="tables m-top-md table-hover table-striped">
								...
								<?php if(is_array($rows)): $k = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($k % 2 );++$k;?><tr class="text-c">
										<td><?php echo ($k); ?></td>
										<td><?php echo ($row["title_one"]); ?></td>
										<td><?php echo ($row["title_two"]); ?></td>
										<td><?php echo ($row["content"]); ?></td>
										<td class="f-14">
											<?php if(in_array('HomeContent/edit',per()) OR user()['id'] == 1): ?><a href="<?php echo U('/HomeContent/edit',array('hid'=>$row['id']));?>" class="btn btn-info">编辑</a><?php endif; ?>
											<?php if(in_array('HomeContent/del',per()) OR user()['id'] == 1): ?><a href="javascript:;" name="<?php echo ($row["id"]); ?>" class="btn btn-infos btndel">删除</a><?php endif; ?>
...
<script>
	...
	$(".btndel").on("click",function(){
		if(confirm("确定要删除账号吗")){
			$.maskajax('post',"<?php echo U('/HomeContent/del');?>",{"id":$(this).attr("name")},function(data){
				if(data == 0) {
					alert('删除出错了');
				}else {
                    location.reload();
