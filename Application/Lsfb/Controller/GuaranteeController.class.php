...
class GuaranteeController extends PublicController
{
    ...
    public function add()
    {
        ...
        if (IS_POST) {
            ...
            $data['time'] = strtotime(I('post.time', '', false));
            $data['content'] = I('post.content', '', false);
            $res = $this->_model->add($data);
            if ($res === false) {
                $this->error(getError($this->_model));
            }
            $this->success('上传成功', U('/Guarantee/index'));
        } else {
            $this->display('edit');
        }
    }
    ...
    public function edit($aid)
    {
        if (IS_POST) {
            ...
            $row['time'] = date('Y-m-d\TH:i:s', $row['time']);
            $this->assign('row', $row);
            $this->display('edit');
        }
    }
