...
class JpushApi extends Model
{
    ...
    public static function tui($telbs, $content, $uu, $dd,$messages ,$lang = 0)
    {
        ...
        file_put_contents("testsjpush.txt",date("Y-m-d H:i:s")."-->".json_encode($aa)."->$telbs\r\n", FILE_APPEND);
        return $aa;
    }
    //推送
    ...
    public static function jpush($title, $message, $data, $titles,$messages=null)
    {
        ...
        $base64 = base64_encode("009927758e8039d6317142f0:83648fd6bfd20ee3c1e2f014");
        $header = array("Authorization:Basic $base64", "Content-Type:application/json");
        if ($data == 1) {
            $param = '{"platform":"all","audience":' . $title . ',"message":{"msg_content":"' . $messages . '","title":"' . $titles . '"},"options" : {"time_to_live" : 60,"apns_production":false}}';
        } elseif ($data == 2) {
            ...
            $param = '{"platform":"all","audience":' . $title . ',"notification" : {
	        	...
	        	"ios" : {
	                 "alert" : "' . $message . '",
	                 "badge":"+1"
	            }},"message":{"msg_content":"' . $messages . '","title":"' . $titles . '"},"options" : {"time_to_live" : 60,"apns_production":false}}';
        }
