...
class Upload {
     ...
     */
    private function getSaveNamed($file,$key) {
        $rule = date("YmdHis_").md5($this->saveName.uniqid());
	...
	private function getSaveName($file,$key) {
		...
		if (empty($rule)) { //保持文件名不变
			/* 解决pathinfo中文文件名BUG */
			$filename = substr(pathinfo("_{$file['name']}", PATHINFO_FILENAME), 1);
			$savename = date("YmdHis_").substr(md5(uniqid()),5,15).$filename;
		} else {
			$savename = date("YmdHis_").substr(md5(uniqid()),5,15).$this->getName($rule, $file['name']).$key;
			if(empty($savename)){
				$this->error = '文件命名规则错误！';
				return false;
			}
