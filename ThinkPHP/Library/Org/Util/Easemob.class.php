...
//		$this->app_name = isset ( $options ['app_name'] ) ? $options ['app_name'] : '';
		...
		$this->org_name = C("appkeyorgName");
		$this->app_name =C("appkeyappName");
	...
	function yy_hxSends($from_user = "admin", $username, $content, $target_type = "users", $ext) {
		$option ['target_type'] = $target_type;
		$option ['target'] = $username;
		$params ['type'] = "txt";
		$params ['msg'] = $content;
	...
	/**
	 *  $post_data['name']='测试2';
	 *	$post_data['description']="保全兄加油";
	 *	$post_data['maxusers']=5000;
	 *	$post_data['owner']='cysj13';
	 *	$post_data['members']=array('cysj12','cysj13');
	 * */
	//聊天室添加
	public function chatrooms($option) {
	...
	//聊天室删除
	public function chatroomsdel($chatid) {
		$url = $this->url . "chatrooms/" . $chatid;
		$access_token = $this->getToken ();
		$header [] = 'Authorization: Bearer ' . $access_token;
		$result = $this->postCurl ( $url, '', $header, $type = "DELETE" );
		return $result;
	...
	//获取聊天室详情(所有的和单个的)
	public function chatroomsw($option) {
		if($option){
			$url = $this->url . "chatrooms/" . $option;
		}else{
			$url = $this->url . "chatrooms";
		}
		$access_token = $this->getToken ();
	...
	//获取用户加入的聊天室
	public function chatroomse($username) {
